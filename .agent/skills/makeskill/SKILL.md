...
- **Cursor 发现**：创建后执行 `bash scripts/link_cursor_skills.sh`，在 `.cursor/skills/` 生成符号链接
- **全局级**：`~/.cursor/skills/<skill-name>/`（跨项目）或 `~/.gemini/antigravity/skills/<skill-name>/`
...
仅创建用户需要的可选目录。完成后运行 `bash scripts/link_cursor_skills.sh` 以便 Cursor 自动加载。

**Cursor 自动触发**：`description` 须含中英文触发词；勿设 `disable-model-invocation: true`（除非仅 `@` 手动调用）。
